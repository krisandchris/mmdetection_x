...
class Panda_tool:
    ...
    @staticmethod
    def remove_val(img_path,lab_path,val_path):
        images = os.listdir(img_path)
        val_images = os.listdir(val_path)
        # 移动源数据中的val部分
        with open(lab_path, 'r', encoding='utf-8') as f:
            labels = json.load(f)
            labels_ed = edict(labels)
        # print(type(labels_ed.images[0].file_name))  :str
        for image in images:
            name_img = image
        # 移动与验证标注中同名的文件
            for ids in labels_ed.images:
                if name_img != ids.file_name:
                    continue
                else:
                    shutil.copy(os.path.join(img_path, name_img), val_path)
        ...
        for image in images:
            ...
            for val_image in val_images:
                if val_image == name_img:
                    os.remove(os.path.join(img_path, name_img))
        ...
        for img_name, v in img_instance.items():
            ...
            for bbox in bboxes:
                ...
                cv2.putText(src, '%d' % label,
                            ...
                            (0, 0, 255), 4)
            print(img_name)
            cv2.imwrite('../user_data/tmp_data/visual_results/'+osp.basename(img_name),src)
            input()
    ...
    def vis_gt_offical(json_path:str, img_root_path:str):
        ...
        :return:
        '''
        with open('../tcdata/Test/image_annos/person_bbox_test_A.json','r') as fr:
        ...
        for k,v in pred_results.items():
            ...
            for bbox in v:
                ...
                cv2.putText(src, '%d|%.3f' % (label,score),
                            ...
                            0.8,
                            (0, 0, 255), 6)
            cv2.imwrite('../user_data/tmp_data/visual_results/'+osp.basename(cur_img_path),src)
            print(cur_img_path)
            input()
    ...
    @staticmethod
    def bbox_json2commited_divided(coco_test_path: str, bbox_json_path: str,
                                    save_json_path: str = '',overlap_factor: list = [512,512],
        ...
        with open('../tcdata/Test/image_annos/person_bbox_test_B.json', 'r') as fr:
            img2wh_id = json.load(fr)
        ...
        for bbox_ins in bbox_json_file:
            # print(img_id2name[bbox_ins['image_id']])
            file_name, (col_cutshape, row_cutshape) = bbox_ins['image_id'], (2560,2560)

            src_file_name = '_'.join(file_name.split('_')[:3])+'.jpg'
            src_file_name = osp.join(root_path_dict[src_file_name.split('_')[1]], src_file_name)
            src_file_name = src_file_name.replace('\\', '/')
        ...
        if show_flag:
            ...
            for img_name, bboxes in images_ins.items():
                print(img_name)
                src = cv2.imread(osp.join('../Tcdata/Test/image_annos/{}'.format(root_path_dict[img_name.split('_')[1]]), img_name))
                print(bboxes)
                ...
                cv2.imwrite('../usr_data/tmp_data/visual_results/' + img_name, src)
                input()
        # 生成官方提交文件
        result = []
        # 对每张图的每个类别做NMS
        ...
        if not save_json_path:
            ...
            save_json_path = osp.join('../prediction_result/', save_name)
        Panda_tool.write2result(result, save_json_path)
    @staticmethod
    def json_adpat(predictions_path, merge_path):
        id_name = {}
        with open(predictions_path) as fr:
            bbox_json = json.load(fr)

            for bbox_num in bbox_json:
                bbox_num['image_id'] = bbox_num['image_id'] + ".jpg"
            id_name = bbox_json
        fr.close()
        with open(merge_path, 'w')as r:
            json.dump(id_name, r, indent=4)
        r.close()
    ...
    def run():
        ...
        4.转为YOLO标注格式
        5.训练模型，调参
        6.生成测试集对应的切片图和coco_json
        7.将生成的bbox.json转为官方提交格式，注意切图场景
        '''
        # Panda_tool.data2coco(src_json_paths=['../tcdata/Train/image_annos/person_bbox_train.json',
        #                                      '../tcdata/Train/image_annos/vehicle_bbox_train.json'],
        #                     save_json_path='../tmp_data/panda__coco_full.json')

        # Panda_tool.split_train_val(json_path='../user_data/tmp_data/panda_round1_coco_full_patches_wh_2560_2560.json',
        #                           pic_path='../user_data/tmp_data/YOLO/images/train')

        # 切图后预测结果调整
        # Panda_tool.json_adpat(predictions_path='runs/test/exp/yolov5m_epoch_74_best_predictions.json',
        #                       merge_path='../prediction_result/merge_split.json')
        # 生成可提交的预测结果
        # Panda_tool.bbox_json2commited_divided(
        #     coco_test_path='../user_data/tmp_data/panda_round1_coco_full_patches_wh_2560_2560_test_B.json',
        #     bbox_json_path='../prediction_result/merge_split.json',
        #     save_json_path='../prediction_result/merge.json', overlap_factor=[512, 512],
        #     nms_thresh=0.6, use_box_voting=True,
        #     show_flag=False)
